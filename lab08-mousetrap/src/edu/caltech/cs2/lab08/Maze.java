...
public class Maze {
    ...
    public Point[] getChildren(Point point) {
        ...
        if (!north[x][y]){
            children[childs] = new Point(x, y+1);
            children[childs].parent = point;
            childs++;
        }
        if (!south[x][y]){
            children[childs] = new Point(x, y-1);
        ...
        if (!east[x][y]){
            children[childs] = new Point(x+1, y);
            children[childs].parent = point;
            childs++;
        }
        if (!west[x][y]){
        ...
        Point[] newChildren = new Point[childs];
    ...
    private void solveDFSRecursive(Point point) {
        ...
        Point[] childs = getChildren(point);
    ...
    public void solveDFSIterative() {
        ...
        while (!done){
            ...
            Point[] childs = getChildren(cur);
            for (Point child : childs){
                if (!child.equals(cur.parent) && !visited.contains(child)){
                    stack.add(child);
                }
            }
            cur = stack.pop();
