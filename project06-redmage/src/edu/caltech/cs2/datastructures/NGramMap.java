...
public class NGramMap {
    ...
    public IPriorityQueue.PQElement<String>[] getCountsAfter(NGram ngram) {
        ...
        IDictionary<IterableString, Integer> values = this.map.get(ngram);
        IPriorityQueue.PQElement<String>[] words = new IPriorityQueue.PQElement[values.size()];
        int idx = 0;
        for (IterableString key : values.keySet()) {
            int priority = values.get(key);
            String keyString = key.toString();
            IPriorityQueue.PQElement<String> toAdd = new IPriorityQueue.PQElement<String>(keyString, priority);
            words[idx] = toAdd;
            idx++;
        }
        return words;
    ...
    public String[] getWordsAfter(NGram ngram, int k) {
        ...
        String[] array = new String[Math.min(counts.length, k)];
        int idx = 0;
        for (int i = 0; i < counts.length; i++) {
            array[i] = counts[idx].data;
            idx++;
        }
        return array;
    }
